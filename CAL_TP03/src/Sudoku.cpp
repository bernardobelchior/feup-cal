...
bool Sudoku::solve()
{
	for(int i = 0; i < 9; i++){
		for(int j = 0; j < 9; j++){
			if(numbers[i][j] != 0){
				columnHasNumber[j][numbers[i][j]] = true;
				lineHasNumber[i][numbers[i][j]] = true;
				block3x3HasNumber[i/3][j/3][numbers[i][j]] = true;
			}
		}
	}
	...
	for(int i = 0; i < 3; i++){
		for(int j = 0; j < 3; j++){
			...
			if(numbers[i][j] == 0){
				for(int k = 1; k < 10; k++){
					if(!columnHasNumber[i][j] && !lineHasNumber[i][j] && !block3x3HasNumber[i/3][j/3][k]) {
						count++;
					}
				}
				if(count < numMin){
